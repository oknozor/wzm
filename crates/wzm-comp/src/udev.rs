...
use crate::state::{
    post_repaint, take_presentation_feedback, Backend, SurfaceDmabufFeedback, WzmState
};
...
use smithay::backend::drm::{
    CreateDrmNodeError, DrmAccessError, DrmDevice, DrmDeviceFd, DrmError, DrmEvent, DrmEventMetadata, DrmNode, DrmSurface, GbmBufferedSurface, NodeType
};
...
impl DmabufHandler for WzmState<UdevData> {
    ...
    fn dmabuf_imported(
            ...
            .is_ok()
        {
            ...
            let _ = notifier.successful::<WzmState<UdevData>>();
        } else {
            notifier.failed();
        }
    }
}
delegate_dmabuf!(WzmState<UdevData>);
...
pub fn run_udev() {
    ...
    let mut state = WzmState::init(display, event_loop.handle(), data, true);
        ...
        .insert_source(notifier, move |event, &mut (), data| match event {
            ...
            SessionEvent::ActivateSession => {
                    ...
                    .map(|(handle, backend)| (*handle, backend))
                {
                    ...
                    if let Some(lease_global) = backend.leasing_global.as_mut() {
                        lease_global.resume::<WzmState<UdevData>>();
                    }
    ...
    let global = dmabuf_state.create_global_with_default_feedback::<WzmState<UdevData>>(
        &display_handle,
        &default_feedback,
    );
...
impl DrmLeaseHandler for WzmState<UdevData> {
...
delegate_drm_lease!(WzmState<UdevData>);
...
impl Drop for SurfaceData {
    fn drop(&mut self) {
        if let Some(global) = self.global.take() {
            self.dh.remove_global::<WzmState<UdevData>>(global);
        }
    }
}
...
impl WzmState<UdevData> {
    fn device_added(&mut self, node: DrmNode, path: &Path) -> Result<(), DeviceAddError> {
        ...
        let registration_token = self
            ...
            .insert_source(
                notifier,
                move |event, metadata, data: &mut WzmState<_>| match event {
        ...
        self.backend_data.backends.insert(
            ...
            BackendData {
                ...
                leasing_global: DrmLeaseState::new::<WzmState<UdevData>>(
                    &self.display_handle,
                    &node,
                )
                .map_err(|err| {
    ...
    fn connector_connected(
        ...
        if non_desktop {
            ...
            if let Some(lease_state) = device.leasing_global.as_mut() {
                lease_state.add_connector::<WzmState<UdevData>>(
                    connector.handle(),
                    output_name,
                    format!("{} {}", make, model),
                );
            ...
            let global = output.create_global::<WzmState<UdevData>>(&self.display_handle);
    ...
    fn device_removed(&mut self, node: DrmNode) {
        ...
        if let Some(mut backend_data) = self.backend_data.backends.remove(&node) {
            if let Some(mut leasing_global) = backend_data.leasing_global.take() {
                leasing_global.disable_global::<WzmState<UdevData>>();
            }
    ...
    fn schedule_initial_render(
        ...
        node: DrmNode,
        crtc: crtc::Handle,
        evt_handle: LoopHandle<'static, WzmState<UdevData>>,
    ) {
        let device = if let Some(device) = self.backend_data.backends.get_mut(&node) {
            device
        } else {
