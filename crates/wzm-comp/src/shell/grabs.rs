...
use smithay::input::pointer::{
    AxisFrame, ButtonEvent, GestureHoldBeginEvent, GestureHoldEndEvent, GesturePinchBeginEvent, GesturePinchEndEvent, GesturePinchUpdateEvent, GestureSwipeBeginEvent, GestureSwipeEndEvent, GestureSwipeUpdateEvent, GrabStartData as PointerGrabStartData, MotionEvent, PointerGrab, PointerInnerHandle, RelativeMotionEvent
};
...
pub struct PointerMoveSurfaceGrab<B: Backend + 'static> {
    pub start_data: PointerGrabStartData<WzmState<B>>,
    pub window: WindowElement,
    pub initial_window_location: Point<i32, Logical>,
}

impl<BackendData: Backend> PointerGrab<WzmState<BackendData>>
    for PointerMoveSurfaceGrab<BackendData>
{
    fn motion(
        &mut self,
        data: &mut WzmState<BackendData>,
        handle: &mut PointerInnerHandle<'_, WzmState<BackendData>>,
    ...
    fn start_data(&self) -> &PointerGrabStartData<WzmState<BackendData>> {
        &self.start_data
    }

    fn unset(&mut self, _data: &mut WzmState<BackendData>) {}
}

pub struct TouchMoveSurfaceGrab<B: Backend + 'static> {
    pub start_data: TouchGrabStartData<WzmState<B>>,
    pub window: WindowElement,
    pub initial_window_location: Point<i32, Logical>,
}

impl<BackendData: Backend> TouchGrab<WzmState<BackendData>> for TouchMoveSurfaceGrab<BackendData> {
    fn down(
        &mut self,
        _data: &mut WzmState<BackendData>,
        _handle: &mut smithay::input::touch::TouchInnerHandle<'_, WzmState<BackendData>>,
        _focus: Option<(
            <WzmState<BackendData> as smithay::input::SeatHandler>::TouchFocus,
            Point<i32, Logical>,
        )>,
    ...
    fn up(
        &mut self,
        data: &mut WzmState<BackendData>,
        handle: &mut smithay::input::touch::TouchInnerHandle<'_, WzmState<BackendData>>,
    ...
    fn start_data(&self) -> &smithay::input::touch::GrabStartData<WzmState<BackendData>> {
        &self.start_data
    }
...
impl<BackendData: Backend> TouchGrab<WzmState<BackendData>>
