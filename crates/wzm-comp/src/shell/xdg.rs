...
use smithay::desktop::{
    find_popup_root_surface, get_popup_toplevel_coords, layer_map_for_output, PopupKeyboardGrab, PopupKind, PopupPointerGrab, PopupUngrabStrategy, Space, Window, WindowSurfaceType
};
...
use super::{
    fullscreen_output_geometry, place_new_window, FullscreenSurface, PointerMoveSurfaceGrab, PointerResizeSurfaceGrab, ResizeData, ResizeEdge, ResizeState, SurfaceData, WindowElement
};

impl<BackendData: Backend> XdgShellHandler for WzmState<BackendData> {
    ...
    fn move_request(&mut self, surface: ToplevelSurface, seat: wl_seat::WlSeat, serial: Serial) {
        let seat: Seat<WzmState<BackendData>> = Seat::from_resource(&seat).unwrap();
        ...
        if let Some(touch) = seat.get_touch() {
            if touch.has_grab(serial) {
                    ...
                    .contains(xdg_toplevel::State::Maximized)
                {
                    ...
                    //
                    // but for wzm needs setting location to pointer location is fine
