...
impl DmabufHandler for WzmState<X11Data> {
    ...
    fn dmabuf_imported(
        ...
        if self
            ...
            .is_ok()
        {
            let _ = notifier.successful::<WzmState<X11Data>>();
        } else {
            notifier.failed();
        }
    }
}
delegate_dmabuf!(WzmState<X11Data>);
...
pub fn run_x11() {
    ...
    let dmabuf_global = dmabuf_state.create_global_with_default_feedback::<WzmState<X11Data>>(
        &display.handle(),
    ...
    let _global = output.create_global::<WzmState<X11Data>>(&display.handle());
