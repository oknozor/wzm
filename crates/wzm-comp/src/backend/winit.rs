...
#[cfg(feature = "debug")]
use smithay::{
    backend::{allocator::Fourcc, renderer::ImportMem}, reexports::winit::raw_window_handle::{HasWindowHandle, RawWindowHandle}
};
...
use crate::drawing::*;
use crate::render::*;
use crate::state::{post_repaint, take_presentation_feedback, Backend, WzmState};
...
impl DmabufHandler for WzmState<WinitData> {
    ...
    fn dmabuf_imported(
        ...
        if self
            ...
            .is_ok()
        {
            let _ = notifier.successful::<WzmState<WinitData>>();
        } else {
            notifier.failed();
        }
    }
}
delegate_dmabuf!(WzmState<WinitData>);
...
pub fn run_winit() {
    ...
    let _global = output.create_global::<WzmState<WinitData>>(&display.handle());
    ...
    let dmabuf_state = if let Some(default_feedback) = dmabuf_default_feedback {
        ...
        let dmabuf_global = dmabuf_state
            .create_global_with_default_feedback::<WzmState<WinitData>>(
                &display.handle(),
                &default_feedback,
            );
        ...
        let mut dmabuf_state = DmabufState::new();
        let dmabuf_global =
            dmabuf_state.create_global::<WzmState<WinitData>>(&display.handle(), dmabuf_formats);
